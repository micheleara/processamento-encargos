...
class KafkaTopicsConfigTest {
    ...
    void setUp() throws Exception {
        ...
        setField(kafkaTopicsConfig, "topicoConfirmacaoContabilDlt", "encargos.contabil.lancamento.response.DLT");
        setField(kafkaTopicsConfig, "topicoConsultaResponseDlt", "encargos.conta.consulta.response.DLT");
    }
    ...
    @Test
    @DisplayName("Deve criar tópico DLT de confirmação contábil com 1 partição")
    void deveCriarTopicConfirmacaoContabilDlt() {
        NewTopic topic = kafkaTopicsConfig.topicConfirmacaoContabilDlt();

        assertNotNull(topic);
        assertEquals("encargos.contabil.lancamento.response.DLT", topic.name());
        assertEquals(1, topic.numPartitions());
        assertEquals(1, topic.replicationFactor());
    }

    @Test
    @DisplayName("Deve criar tópico DLT de consulta response com 1 partição")
    void deveCriarTopicConsultaResponseDlt() {
        NewTopic topic = kafkaTopicsConfig.topicConsultaResponseDlt();

        assertNotNull(topic);
        assertEquals("encargos.conta.consulta.response.DLT", topic.name());
        assertEquals(1, topic.numPartitions());
        assertEquals(1, topic.replicationFactor());
    }

    @Test
    @DisplayName("Todos os sete tópicos devem ter nomes distintos")
    void todosSeteTopicosDevemTerNomesDistintos() {
        ...
        NewTopic t6 = kafkaTopicsConfig.topicConfirmacaoContabilDlt();
        NewTopic t7 = kafkaTopicsConfig.topicConsultaResponseDlt();

        long nomes = java.util.Set.of(
                t1.name(), t2.name(), t3.name(), t4.name(), t5.name(), t6.name(), t7.name()).size();
        assertEquals(7, nomes);
    }
}